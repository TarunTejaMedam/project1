...
export class Directives {
  ...
  statusChange(event: any){
    if(event.target.value == 'Completed') this.filteredTasks = this.tasks.filter(task => task.completed);
    else if(event.target.value == 'Pending') this.filteredTasks = this.tasks.filter(task => !task.completed);
  ...
  categories = this.products.reduce((list: string[], prod) => {
    if(!list.includes(prod.category)) list.push(prod.category);
    return list;
  }, [])

  filteredProducts = [...this.products];
  filterCat(event: any) {
    if(event.target.value == 'All') this.filteredProducts = [...this.products];
    else{
      this.filteredProducts = this.products.filter(prod => prod.category == event.target.value)
    }
  }
  ...
  qtyControl(item: any, ctrl: string){
    if(ctrl == 'I') item.qty++;
    else if(ctrl == 'D' && item.qty > 0) item.qty--;
  }

  totalPrice(){
    return this.cartItems.reduce((sum, items) => sum += items.price*items.qty, 0)
  }
  ...
  rowStyle(emp: any){
    ...
    if(emp.role == 'Manager') classes = 'manager';
    else if(emp.role == 'Developer') classes = 'developer';
    else if(emp.role == 'Tester') classes = 'tester';

    if(emp.status == 'Inactive') classes += ' inactive';

    return classes;
  }

  gr = {"color": "green", "background-color": "lightgreen"};
  gor = {"color": "green", "background-color": "orange"};
  ror = {"color" : "red", "background-color": "orange"};
  rd = {"color" : "red", "background-color": "lightcoral"};

  badgeClr(sal: number){
    if(sal > 70000) {
      return this.gr;
    }
    else if(sal < 70000 && sal > 40000) {
      if(sal > 50000) return this.gor;
      else return this.ror;
    }
    else if(sal < 40000) return this.rd;
    return {};
  }
}
